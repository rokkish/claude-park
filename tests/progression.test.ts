...
> = {
  ...
  // 4-1: ゴールは橋の先の床の上 (35,12)。テレポートなので壁もブロックも関係しない。
  "stage-12": { goal: { x: 35, y: 12 } },
  // 4-2: ゴールは左上の棚 (2,7)。
  "stage-13": { goal: { x: 2, y: 7 } },
  // 4-3: ゴールは右端の床 (74,12)。
  "stage-14": { goal: { x: 74, y: 12 } },
  // 4-4: ゴールは奥の通路の床 (74,12)。
  "stage-15": { goal: { x: 74, y: 12 } },
};
...
describe("ステージ進行", () => {
  it("16ステージ（ワールド1が4本、ワールド2が5本、ワールド3が3本、ワールド4が4本）が登録されている", () => {
  ...
  it("isAllCleared は最終ステージ(4-4)をクリアした瞬間だけ真になる", () => {
    const { game, input, step } = newGame();
    const nonFinalIds = [
      "stage-01",
      "stage-02",
    ...
    // 途中の15ステージは、クリアしても isAllCleared はまだ立たない
    ...
    // 最終ステージ (stage-15 = 4-4) クリアで全踏破
    forceClear(game, step);
    expect(game.stage.data.id).toBe("stage-15");
