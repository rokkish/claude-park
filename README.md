...
ワールド4の4本目 **4-4「Skylight」** を追加。全16ステージになりました。

**ボールの規則の変更**: 狙いの向きの決め方を、中心のずれの比率ではなく**状態**（地上か
空中か）で決めるようにしました。向きは3種類。立って横から当てれば水平、跳んで横から
当てれば必ず45°斜め上、真下から当てれば真上。旧ルールは接触した瞬間の人の高さで角度が
27°〜49°の間でぶれて狙えませんでしたが、45°に固定したことで幾何が決まり、立つ位置で
狙えるようになりました。あわせて、飛んでいる球は箱に当たっても消えるようにしました
（従来は素通りしていました）。

- **気づき①「頭の高さの球は跳ばないと触れない。返せば必ず斜め」**
- **気づき②「穴は自分の1.5タイル後ろ（球が来た側）に開く。立つ位置で狙う」**
- **気づき③「箱は穴の端に置く。先に登る人は左から。後の人は箱を1つずらして右から」**
- **気づき④「奥は4-2と同じ厚い壁（隠れた2画面目）」**

### v1.7.0 — 2026-09-18

追従カメラを導入し、ワールド4の3本目 **4-3「Door」** を追加。全15ステージになりました。
既存ステージはすべて1画面に収まっているため見た目はそのままで、4-3 だけが初めて
画面より広い（80×18タイル）ステージになります。

- **エンジン変更①「球が飛んでいる間はカメラの対象」** — ギミックが
  `cameraTarget?(): AABB | null` を実装すると、返した矩形が追従カメラの対象に加わります。
  ボールが実装しており、打った球が先の地形を偵察するようにカメラが引きます。
- **エンジン変更②「閉じた扉や足場もボールを止める」** — `GimmickContext.solids`
  （前ステップで確定したギミック由来の Solid）にボールが当たると、壁と同じく消えます。
  人が跳び越えられる1タイルの扉でも、ボールだけは止められます。

- **気づき①「扉はボールも止める」** — 相方が板を踏んでいる間だけ球が通る。板は棚の上、
  ボールの通り道の外にある
- **気づき②「厚い壁は3回」** — 4-2 と同じ、床に立ったまま同じ行を3回打ち返して貫通する
- **気づき③「4発目の前に扉を閉める」** — 板から降りるだけで球を止められる。避けなくてよい
- **気づき④「スクロール＝球の偵察」** — 壁の向こうは見えないが、球を打てばカメラが追って
  先の地形が一瞬見える

### v1.6.0 — 2026-09-18

ワールド4の2本目 **4-2「Chimney」** を追加。全14ステージになりました。

4-1 の壁は1回打ち返せば貫通しましたが、4-2 の壁は3列×4段の厚みがあり、**同じ列を3回**
打ち返さないと抜けません。加えて、ボールを2個置けます。床のボールAは横専用、天井から
垂れた「煙突」の下に浮かせたボールBは、真下から突き上げると縦専用になります。

- **気づき①「同じ列を3回返す」** — 厚い壁は1回では抜けない。床に立ったまま同じ行を
  3回打ち返して初めて貫通する
- **気づき②「浮いたボールは真下から打つ」** — 浮いたボールBの真下に立って下から当てると
  垂直に飛び、廊下の穴を抜けて煙突を根元から1個ずつ崩す
- **気づき③「斜めに返すと橋が減る」** — ジャンプ中に返すと斜めに飛び、廊下の橋を下から
  壊してしまう。壊しすぎるとゴール直前の足場を失う

**ボールの規則の変更**: 狙いの向きが**軸に揃う**ようにしました。打った人の中心から
ボールの中心への向きのうち、小さい方の成分が大きい方の半分未満なら 0 にする、というだけの
変更ですが、これで「床のボールは横」「浮いたボールは縦」と役割が分かれ、1ステージに
ボールを複数置いて使い分けられます。

### v1.5.0 — 2026-09-18

World 4（キーアイデア: **ブロック崩し**）の最初のステージ **4-1「Breakout」** を追加。
全13ステージ・4ワールドになりました。

ボールに人が触れると、その人から離れる向きに飛びます。ブロック（`B`）に当たると壊して跳ね返り、
人に当たると跳ね返り、壁に当たると消えて元の位置に戻ります。**壁の根元を壊さないと先へ進めず、
橋を壊しすぎるとゴールに届きません。**

- **気づき①「線上の人は板になる」** — 右へ打つには左に回り込む。相方が線上に立っていると
  ボールは2人の間を往復して壁に届かないので、相方は跳んで通す
- **気づき②「壁を抜けたら返さない」** — 戻ってきたボールを返すと、次に壊れるのは橋。
  2人とも跳んでやり過ごせば、ボールは壁で消えて元の位置に戻る

ブロックは新しいギミックではなく**地形の一種**です。当たり判定は壁と同じ経路で動き、
壊す＝グリッドを書き換えるだけで、`R` でのやり直しで地形ごと戻ります。

### v1.4.0 — 2026-07-26

高難易度ステージ **2-4「Tower」** を追加。全11ステージになり、ワールド2も4本になりました。
これまで下半分だけだったステージが、**画面の上半分まで使う縦構成**になっています。

- **気づき①「足場は道のりの半分しか行かない」** — 1つの信号が上がるリフトと下がるリフトを
  同時に動かす。両方が中段に居るのは板を踏んでいる間だけで、そのすれ違いの一瞬に乗り継ぐ
- **気づき②「橋は足場そのもの」** — 上の板は島の上にしかなく、島と棚をつなぐ橋は足場。
  板を踏むと足場が降りて橋が消えるので、押した本人が島に取り残される

中段に2人が並ぶことは構造上あり得ないので、中段より上は単独ジャンプ(62.5px)だけで
検算できます。二段ジャンプ(149px)を考慮するのは地上からの跳躍だけです。
